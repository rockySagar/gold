 ...
 * author : Rakesh  
 * Date : 07-Oct-2021
 * Description : Organisation database operations
 */

 const Organisation = require('./model')

 module.exports = class OrganisationClass {
     static create(data) {
         return new Promise(async (resolve, reject) => {
             try {
                 let response = await new Organisation(data).save()
                 resolve(response)
             } catch (error) {
                 reject(error)
             }
     ...
     static updateOne(filter, update, options = {}) {
         return new Promise(async (resolve, reject) => {
             try {
                 const updateResponse = await Organisation.updateOne(filter, update, options)
                 if (
     ...
     static findOne(filter, projection = {}) {
         return new Promise(async (resolve, reject) => {
             try {
                 const sessionData = await Organisation.findOne(filter, projection).lean()
                 resolve(sessionData)
             } catch (error) {
                 reject(error)
             }
     ...
     static findById(id) {
         return new Promise(async (resolve, reject) => {
             try {
                 const session = await Organisation.findOne({ _id: id, deleted: false, status: { $ne: 'cancelled' } }).lean()
                 resolve(session)
             } catch (error) {
                 reject(error)
             }
     ...
     static findAll(page, limit, search, filters) {
         return new Promise(async (resolve, reject) => {
             try {
                 let sessionData = await Organisation.aggregate([
                     {
                         $match: {
     ...
     static find(filter, projection = {}) {
         return new Promise(async (resolve, reject) => {
             try {
                 let sessionData = await Organisation.find(filter, projection).lean()
                 resolve(sessionData)
             } catch (error) {
                 reject(error)
             }
     ...
     static update(filter, update, options = {}) {
         return new Promise(async (resolve, reject) => {
             try {
                 const updateResponse = await Organisation.update(filter, update, options)
                 if (
                     (updateResponse.n === 1 && updateResponse.nModified === 1) ||
                     (updateResponse.matchedCount === 1 && updateResponse.modifiedCount === 1)
                 ) {
